...
function chart(column, filterBy, groupBy) {
	...
	var chartTop = $('.streamgraph.'+groupBy+'.'+filterBy).offset().top;
	...
	var svg = d3.select(".streamgraph."+groupBy+'.'+filterBy).append("svg")
	...
	function legend(layers){
		...
		$('.streamgraph.'+groupBy+'.'+filterBy).prepend('<div class="legend"><div class="title">'+titler(filterBy,groupBy)+'</div></div>');
		$('.legend').hide();
		var legend = []
	...
	function parse(data){
		...
		// based on the filters provided in the .streamgraph div (see the html file)
		var filter;
		var searchObj = {};
	...
	function loadTheStreamgraph(data){
		...
		var vertical = d3.select(".streamgraph."+groupBy+'.'+filterBy)
			.append("div")
			.attr("class", "remove")
			.style("position", "absolute")
			.style("z-index", "19")
		...
		d3.select(".streamgraph."+groupBy+'.'+filterBy)
	...
	var column = $('.streamgraph').attr("column");
